...
const filteredTodos=todos.filter((item: { year: any; month: any; day: any; })=>{
    let Date=`${item.year}-${item.month}-${item.day}`;
    return Date===today_date;  //오늘 날짜랑 내가 조합한 날짜랑 같은 것만 나오게 return
});

console.log('fik',filteredTodos)
const titles= filteredTodos.map((item: { events: any[]; })=>item.events.map(event=>event.title))

todoEle && (todoEle.textContent = titles.length > 0 
    ? titles.join(', ')  // title이 있을 때 
