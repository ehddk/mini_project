...
window.onload = function () {
  const loading =
    (document.querySelector(".loading") as HTMLDivElement) || null;
  const weatherDataDiv =
    (document.querySelector(".main_weather") as HTMLDivElement) || null;
...
function updateWeatherUI(
  ...
  seoulData: any,
  gangwonData: any,
  jeonbWeatherData: any,
  jeonNWeatherData: any
) {
  const today = new Date();
  let year = today.getFullYear();
  let month = (today.getMonth() + 1).toString().padStart(2, "0"); // 월을 2자리로 표시
  ...
  const iconEle = document.getElementById(
    "mainIcon"
  ...
  const areaIconAdrs = `https://openweathermap.org/img/wn/${Icon}@2x.png`;
  ...
  const seoulIcon = document.getElementById(
    "seoul_icon"
  ) as HTMLImageElement | null;
  const gangIcon = document.getElementById(
    "gang_icon"
  ) as HTMLImageElement | null;
  const seoulAdrs = `https://openweathermap.org/img/wn/${seoul_icon}@2x.png`;
  const gangAdrs = `https://openweathermap.org/img/wn/${gang_icon}@2x.png`;

  /**전북,전남 엘리먼트  */
  const tempEle = document.getElementById("jeonb");
  const area1_icon = document.getElementById(
    "jeonb_icon"
  ) as HTMLImageElement | null;
  const desEle = document.getElementById("description");
  const jeonNEle = document.getElementById("jeonN");
  const jeonNIcon = document.getElementById(
    "jeonN_icon"
  ) as HTMLImageElement | null;
  const jeonNDes = document.getElementById("jeonDes");
  const jeonNIconAdrs = `https://openweathermap.org/img/wn/${jeonN_icon}@2x.png`;
  ...
  gangwonEle &&
    (gangwonEle.textContent = `${(Gangwon_temp - 273.15).toFixed(2)}도`); //강원도 현재 온도 표시
  ...
  switch (true) {
    ...
    case calcTemp > 10:
      messageEle &&
        (messageEle.textContent = "바람이 불 수 있으며,아우터를 꼭 챙기세요.");
      break;
    case calcTemp < -5:
      messageEle &&
        (messageEle.textContent =
          "날씨가 몹시 추우니 보온에 각별히 유의해주세요!");
      break;
    default:
      messageEle &&
        (messageEle.textContent = "몸관리와 한파 예방에 더욱더 신경써주세요.");
  }
